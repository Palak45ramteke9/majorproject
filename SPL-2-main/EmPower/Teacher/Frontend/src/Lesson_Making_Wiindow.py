...
class Lesson_Making_Window(QMainWindow):  # Home extends QMainWindow
    ...
    def getFolderDetails(self):
        ...
        # read the folders
        if(os.path.exists('Lessons/Modules')):
            moduleNames = os.listdir('Lessons/Modules')

            for folder in moduleNames:
                if 'মডিউল' in folder: 
                    items = os.listdir('Lessons/Modules/' + folder)

                    for item in items:
                        if item == 'content.json':
                            with open('Lessons/Modules/' + folder + '/' + item) as f:
                                data = json.load(f)
    ...
    def make_lesson(self):
        ...
        sub_folders = os.listdir('Lessons/Lessons')
        ...
        folder_path = 'Lessons/Lessons/Lesson' + str(lesson_serial_no + 1)
        ...
        for row in range(model.rowCount()):
            ...
            if 'মডিউল' in item: 
                sub_folder_name = 'Lessons/Lessons/Lesson_' + str(lesson_serial_no + 1) + '/' + item
                ...
                try:
                    ...
                    continue
                shutil.copytree('Lessons/modules/' + item, sub_folder_name, dirs_exist_ok=True)
        ...
        show_success_message('Lesson created', 'Lesson created. Check the Lessons folder to view the lesson')
     
    def show_lessons(self):
        
        # open lesson folder  
        os.startfile('Lessons\Modules')
