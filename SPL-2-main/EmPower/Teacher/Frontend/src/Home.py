...
class Home(QMainWindow):  # Home extends QMainWindow
    ...
    def lesson_making_page(self):
        ...
        # set window icon and title
        self.setWindowIcon(QIcon("Frontend/Images/primary_logo.png"))
        self.setWindowTitle("Create lessons")
    ...
    def lesson_assigning_page(self):
        ...
        # set window icon and title
        self.setWindowIcon(QIcon("Frontend/Images/primary_logo.png"))
        self.setWindowTitle("List of lesson assignments")
    ...
    def save_sequence_details(self):
        ...
        if image_description == "" or image_set == "":
            show_warning_message("Field incomplete", "Fill all fields")
            return
        ...
        if not os.path.exists('Lessons/Sequence_Images/{}'.format(image_set)) and not image_set in existing_folders:
            ...
            show_warning_message(
                "set name already used", "A set with this name is already used, use another name")
            return
        ...
        if self.sequence_image_count >= 3:
            ...
            show_confirmation_message(
                "Completed", "{} set 4 images selected, can add new images to another set".format(image_set))
            ...
            show_warning_message("Image selection not done", "More {} images to be selected".format(
    ...
    def select_puzzle_image(self):
        ...
        try:
            ...
            names += "To save the image, write the set number above and \'Save\' Click the button."
            ...
            for file_name in file_names:
                ...
                #         self.home.task_puzzle_image_lbl.setText("Image {} Saved in the folder.{} Select the image again to save in a new folder.".format(folder_name,'\n'))

                #     except:
                #         os.mkdir('Lessons/Puzzle_Images/{}'.format(folder_name))
                #         shutil.copy2(file_name, 'Lessons/Puzzle_Images/{}/'.format(folder_name))
                #         self.home.task_puzzle_image_lbl.setText("The image has been saved in the {} folder. {} Please select the image again to save it in a new folder.".format(folder_name,'\n'))
                # else:
    ...
    def save_puzzle_set(self):
        ...
        if os.path.exists('Lessons/Puzzle_Images/{}'.format(puzzle_set)) == False:
            ...
            self.home.task_puzzle_image_lbl.setText("image{} The image has been saved in the folder. {} Please select a new image to save in a new folder.".format(
                self.home.task_puzzle_q_set_lbl.text(), '\n'))

            self.home.task_puzzle_q_set_lbl.clear()
            show_success_message(
                "Image has been saved successfully.", "Saving Completed")

        else:
            show_warning_message("The folder has already been created," "{} folder has already been created! Please select a new set number.".format(
                self.home.task_puzzle_q_set_lbl.text()))
    ...
    def create_mcq_set(self):
        ...
        if self.home.task_mcq_set_no_edit.toPlainText() == "":
            show_confirmation_message(
                "Set number is missing", "Please provide the set number and click on the 'New Question Set' button.")
            return False
        ...
        if os.path.exists(f"Lessons/MCQ_Questions/{set_no}"):
            ...
            show_warning_message(
                "Please re-enter the set name. A set with this name already exists. The currently existing sets are: {}".format(sets))
            return False
        else:
        ...
        # basic instruction
        show_confirmation_message(
            "Create Question Set", "Now provide the question and possible answers. To create a new question, click on the 'Next Question' button.")
    ...
    def finish_mcq_set(self):
        ...
        if self.mcq_questions == {}:
            show_warning_message(
                "No questions", "No questions whose answers can be saved")
            return False
    ...
    def upload_mcq_image(self):
        ...
        img_path = QFileDialog.getOpenFileName(
            self, "Select the image", "", "Image Files (*.png *.jpg *.jpeg)")[0]
        if img_path != '':
    ...
    def next_mcq_question(self):
        ...
        if question == "" or option_1 == "" or option_2 == "" or option_3 == "" or option_4 == "" or correct_answer == "":
            show_warning_message("Complete the question",
                                 "Provide all options")
            return
    ...
    def load_performance_report(self):
        ...
        if self.home.performance_std_id_cmb.currentText() == "Select":
            show_warning_message("Student not selected",
                                 "Select the student and view the performance report")
        ...
        try:
            ...
            show_confirmation_message(
                "Wait!", "Generating performance report")
