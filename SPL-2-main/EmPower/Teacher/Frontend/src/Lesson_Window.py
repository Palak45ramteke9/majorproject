...
class Lesson_Window(QMainWindow):  # Home extends QMainWindow
    ...
    def __init__(self, ui_object):
        ...
        # dictionary
        self.categories = {
            1: 'Name_learning(Noun)',
            2: 'Verb_learning(Verb)',
            3: 'relationship_learning(Association)',
            4: 'Action_learning(Activity)'
        }
    ...
    def create_lesson(self):
        ...
        custom_form.setWindowTitle("Create a new lesson")
    ...
    def record_audio(self):
        ...
        custom_form.setWindowIcon(
            QIcon("../Teacher/Frontend/Images/primary_logo.png"))
        custom_form.setWindowTitle("Record the audio")
    ...
    def save_lesson_content(self, childObj):
        ...
        if self.category_id == 0:
            show_warning_message("Warning!!", "Select the category")
            return

        # !lesson id and topic must be selected
        if self.lesson_id == "" or self.lesson_topic == "":
            show_warning_message("Warning!!", "Enter lesson name and lesson number")
            return

        # make a folder
        self.folder_name = self.categories[self.category_id] + \
            '_Module_' + self.lesson_id
        self.folder_location = 'Lessons/modules/' + self.folder_name
        ...
        if self.media_file_name is None:
            show_warning_message("Warning", "Select Image/Video")
            return
        ...
        if self.media_file_name.split('.')[1] not in self.videoFormat:
            ...
            if self.audio_location is None:
                show_warning_message("Warning", "Select Audio")
                return
        ...
        show_success_message("Success!!", "Text saved, now click on the reload button at the bottom of the screen")
    ...
    def on_category_changed(self, index):
        ...
        if len(self.lesson_elements) > 0:
            ...
            show_confirmation_message("No lessons added", "No lessons have been added to this category yet")
    ...
    def on_lsn_creating_category_changed(self, index):
        ...
        if len(self.lesson_elements) > 0:
            ...
            self.form.lbl_lsn_cat_status.setText(
                f"Total number of lessons in this category: {category_wise_lesson}, new lesson {category_wise_lesson+1}Start from ")

        else:
            self.form.lbl_lsn_cat_status.setText(
                f"There are no lessons in this category yet, start from new lesson 1")
